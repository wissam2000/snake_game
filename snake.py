...
class SNAKE:
    ...
    def update_head_graphics(self):
        head_tilt = self.body[1] - self.body[0]
        if head_tilt == Vector2(1, 0):
            self.head = self.head_left
        elif head_tilt == Vector2(-1, 0):
            self.head = self.head_right
        elif head_tilt == Vector2(0, 1):
            self.head = self.head_up
        elif head_tilt == Vector2(0, -1):
            self.head = self.head_down

    def update_tail_graphics(self):
        tail_tilt = self.body[-2] - self.body[-1]
        if tail_tilt == Vector2(1, 0):
            self.tail = self.tail_left
        elif tail_tilt == Vector2(-1, 0):
            self.tail = self.tail_right
        elif tail_tilt == Vector2(0, 1):
...
class MAIN:
    ...
    def draw_score(self):
        ...
        bg_rect = pygame.Rect(apple_rect.left, apple_rect.top,
                              apple_rect.width+score_rect.width + 8, apple_rect.height)

        pygame.draw.rect(screen, (167, 209, 61), bg_rect)
        pygame.draw.rect(screen, (56, 180, 12), bg_rect, 2)
